...
parser.add_argument('-l', '--lloc', default='upper right')
parser.add_argument('--range', dest='range', type=int, help='', default=2400)
...
fig = sns.boxplot(y="category", x=args.type, hue='category', orient='y', palette=palette_table,
                   data=mean_data, dodge=False, linewidth=0.5, showfliers = False,
                     width=0.5, legend=None, ax=ax[0], log_scale=log_scale)
fig.set(xlabel=None)
fig.set(ylabel=None)
fig.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: '{:g}'.format(x)))
...
if args.type == 'avg_wait_time':
    fig.set_yscale('log', base=10)
    fig.set_ylim(0.2, 80)
    fig.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: '{:g}'.format(x)))  # 将坐标轴label设定为十进制
