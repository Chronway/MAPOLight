...
if not args.temp:  # 不使用缓存
    ...
    def post_process(data):
        ...
        # data['linestyle'] = ""
        # data.loc[data['category'] == 'MAPPO/5%', 'linestyle'] = ":"  # 线条样式
...
fig = sns.boxplot(y="category", x=args.type, hue='category', orient='y', palette=palette_table, 
                  data=mean_data, linewidth=0.5, showfliers = False, width=0.5, legend=None, ax=ax[0], saturation=1,
                  log_scale=log_scale)
fig.set(xlabel=None)
fig.set(ylabel=None)
...
if args.type == 'avg_wait_time':
    ...
    fig.set_ylim(0.2, 1000)
    fig.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: '{:g}'.format(x)))  # 将坐标轴label设定为十进制
elif args.type == 'total_stopped':
    fig.set_yscale('log', base=2)
    fig.set_ylim(0.5, 100)
    fig.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: '{:g}'.format(x)))
