...
parser.add_argument('-a', '--arch', dest='arch', help='', default=['a3c', 'ppo', 'sac', "maxpressure", "uniform"], nargs='+')
parser.add_argument('-p', '--path', dest='path', help='', default=["1.0", "0.2", "0.1", "0.05", "maxpressure", "uniform"], nargs='+')
...
if __name__ == '__main__':
    ...
    for p in args.path:
        for a in args.arch:
            if (d := read_bunch(p, a, args.num)) is not None:
                tbl = pd.concat([tbl, d])
