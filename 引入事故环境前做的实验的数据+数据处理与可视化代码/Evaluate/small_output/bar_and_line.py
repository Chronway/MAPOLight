...
parser.add_argument('-a', '--arch', dest='arch', help='', default=["maxpressure", "websters", "fixed_time"], nargs='+')
# parser.add_argument('-g', '--graph', dest='graph', help='', default=['line', 'box'])
parser.add_argument('-p', '--path', dest='path', help='', default=["2x2", "single", "double", "4x4", "5x5", "real"], nargs='+')
...
tbl['method/pr'] = tbl['method/pr'].map(lambda a: a if a != 'Fixed time/fixed' else 'Fixed time')
...
def div_intersection_num(r):
    # return r['total_stopped'] / 8
    if 'single' in r['pr']:
        return r['total_stopped']
    if 'double' in r['pr']:
        return r['total_stopped'] / 2
    if '2x2' in r['pr']:
        return r['total_stopped'] / 4
    if '4x4' in r['pr']:
        return r['total_stopped'] / 16 
    if '5x5' in r['pr']:
        return r['total_stopped'] / 25
    if 'real' in r['pr']:
        return r['total_stopped'] / 8

if args.type == 'total_stopped':
    # print(tbl.apply(lambda r: div_num(r),axis=1))
    tbl['total_stopped'] = tbl.apply(lambda r: div_intersection_num(r),axis=1)
...
fig = sns.barplot(y="method/pr", x=args.type, orient='y', color='#AAAAAA', data=tbl, dodge=False, linewidth=0.5, width=0.5, legend=None, ax=ax[0])
fig.set(xlabel=None)
fig.set(ylabel=None)
...
def div_intersection_num(r):
    # return r['total_stopped'] / 8
    if 'single' in r['method']:
        return r['total_stopped']
    if 'double' in r['method']:
        return r['total_stopped'] / 2
    if '2x2' in r['method']:
        return r['total_stopped'] / 4
    if '4x4' in r['method']:
...
if args.type == 'total_stopped':
    # print(tbl.apply(lambda r: div_num(r),axis=1))
    data['total_stopped'] = data.apply(lambda r: div_intersection_num(r),axis=1)
