...
def getOptions(args=None):
    ...
    optParser.add_option(
        "--label", category="processing", help="plot label (default input file name"
    )
    optParser.add_option(
        "--invert-yaxis",
        category="processing",
    ...
    optParser.add_option(
        ...
        help="FCD input file(s)",
    )
    optParser.add_argument("--limit", type=int)
    ...
    if options.show:
        sys.stderr.write(
            "Option --show is now set by default and will be removed in the future."
...
def main(options):
    ...
    if len(options.ttype) == 2 and xdata in KEYS and ydata in KEYS:
        xLabel = KEYS[xdata]
        yLabel = KEYS[ydata]
        plt.xlabel(xLabel)
        plt.ylabel(yLabel)
    ...
    for fileIndex, fcdfile in enumerate(options.fcdfiles):
        ...
        for timestep, vehicle in parse_fast_nested(
            fcdfile, "timestep", ["time"], element, attrs
        ):
            totalVehs += 1
            vehID = vehicle.id
        ...
        if totalVehs == 0 or filteredVehs == 0 or options.verbose:
            print(
                "Found %s datapoints in %s and kept %s"
